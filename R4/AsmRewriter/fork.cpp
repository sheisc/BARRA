/*****************************************************************
    usage:
        LD_PRELOAD=/home/iron/src/R4/AsmRewriter/fork.so ./main
******************************************************************/
#include <unistd.h>
#include <dlfcn.h> 
#include <stdio.h>
...
extern "C" unsigned long au_edu_unsw_randomize(void);
...
char *gets(char *s){
    ...
    //DEBUG_OUTPUT(printf("gets() hooked.\n"));
    au_edu_unsw_randomize();
    return _gets(s);
}
...
pid_t fork(void){
    ...
    if(pid == 0){ // child process
        au_edu_unsw_set_main_pid(getpid());
        au_edu_unsw_randomize();
    }
    return pid;
}
...
int pthread_create(pthread_t *thread, const pthread_attr_t *attr,
    ...
    DEBUG_OUTPUT(printf("stacksize = 0x%lx \n",stacksize));
    //FIXME
    stacksize += SS_OFFSET;
    if(!attr){
        attr = &threadAttr;
    ...
    DEBUG_OUTPUT(printf("stacksize = 0x%lx \n",stacksize));
    DEBUG_OUTPUT(printf("pthread_create(0x%lx) is hijacked.\n",(unsigned long)start_routine));
    /*
        WARNING
